...
def bipart(D, inds, rho, l, iters = 100):
    ...
    for i in range(iters):
        ...
        loss.append(objective(D, P, Z_0, Z_1, rho, l))

    return P.reshape((-1, )) #, loss




def objective(D, P, Z1, Z2, rho, l):
    ...
    D1 = D * P.reshape((-1, 1))
    D2 = D * (1-P).reshape((-1, 1))
    
    return ((D1 - D1@Z1) ** 2).sum() + ((D2 - D2@Z2) ** 2).sum() + l * margin(P)
